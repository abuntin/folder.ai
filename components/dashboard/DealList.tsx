...
import { DashboardListItem } from "./DealListItem";
import { typeOptions } from "./DealType";
...
const applyFilters = (deals: Deal[], filters: Filters): Deal[] => {
...
export const DashboardList: React.FC<DashboardListProps> = (props) => {
  ...
  React.useEffect(() => {
    const _deal =
      activeDealId === ""
        ? null
        : deals.filter((deal) => deal.id === activeDealId)[0];
  ...
  const paginatedDeals = applyPagination(filteredDeals, page, limit);
